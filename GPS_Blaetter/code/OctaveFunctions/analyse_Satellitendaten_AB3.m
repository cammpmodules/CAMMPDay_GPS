function [delta_tS,SK, dS, SP, delta_tE, GK, VGK] = analyse_Satellitendaten_AB3(tE, tS, Ephemeriden, Signalstaerken)
...
[SP, delta_tE] = berechne_Empfaengerkoordinaten(SK,dS,StartP,Signalstaerken);
...
function [SP, delta_tE]=berechne_Empfaengerkoordinaten(SK,dS,StartP,Signalstaerken)
% Aufstellen des Gleichungssystems
Gleichungssystem = @(xE,yE,zE,Delta_dS) sqrt(Signalstaerken).*(sqrt((SK(1,:)-xE).^2+(SK(2,:)-yE).^2+(SK(3,:)-zE).^2)-(dS-Delta_dS));
